...
class User
{
    ...
    /*
     * Проверка работоспособности получения полей из АИС "ДОКА"
     * return array()
     */
    public static function getDOKAUsers()
    {
        $sql = 'SELECT * FROM user ORDER BY user.UID LIMIT 20 OFFSET 0';
        $db = Database::getDokaConnection();
        $result = $db->query($sql);
        $users = null;
        $i = 0;
